...
public class AutorizaReservaBean extends JavaBean {
	...
	this.listaReservaVO = reservaService.listaReservasPendentes(loginBean.getCampus(), autorizador);
    }
	...
	if (listaReservasCanceladas.size() > 0 || listaReservasAutorizadas.size() > 0) {

	    try {
		if (listaReservasAutorizadas.size() > 0) {
		    reservaService.alterar(loginBean.getCampus(), loginBean.getPessoaLogin(), listaReservasAutorizadas,
			    "Autorização de reservas");
		}

		if (listaReservasCanceladas.size() > 0) {
		    reservaService.alterar(loginBean.getCampus(), loginBean.getPessoaLogin(), listaReservasCanceladas,
			    "Cancelamento de reservas");
		}

		addInfoMessage("Autorização", "Todas as autorizações e cancelamentos foram gravadas com sucesso!");
	    } catch (ExisteReservaConcorrenteException e) {
		addErrorMessage("Autorização", "Gravação de autorizações não permitida.");
	...
	try {
		...
		emailService.enviaEmailConfirmacao(loginBean.getCampus(), r, emails);
	    }
	...
	try {
		...
		emailService.enviaEmailCancelamento(r.getIdCampus(), r, emails, loginBean.getPessoaLogin(),
	...
	Calendar itemStart = DateTimeUtils.getCalendarFromDates(item.getReserva().getData(),
		item.getReserva().getHoraInicio());
	Calendar itemEnd = DateTimeUtils.getCalendarFromDates(item.getReserva().getData(),
		item.getReserva().getHoraFim());

	for (ReservaVO vo : listaReservaVO) {
	    if (vo.getIdReserva() != item.getIdReserva() && vo.getReserva().getIdItemReserva()
		    .getIdItemReserva() == item.getReserva().getIdItemReserva().getIdItemReserva()) {
		// Verifica todas as reservas conflitantes e marca para exclusão
		Calendar voStart = DateTimeUtils.getCalendarFromDates(vo.getReserva().getData(),
